...
export function SceneNavSidebar({ lines, onScrollToScene, isOpen, onToggle }: SceneNavSidebarProps) {
  ...
  if (!isOpen) {
    return (
      // Fill the panel width given by ResizablePanel
      <div className="w-full h-full border-r border-border bg-sidebar flex flex-col items-center py-2">
  ...
  return (
    // w-full h-full: fills exactly what ResizablePanel gives us — no hardcoded 260px
    <div className="w-full h-full border-r border-border bg-sidebar flex flex-col">
      ...
      {/* ScrollArea needs to flex-1 and have a defined height to scroll */}
          ...
          <div className="p-2 space-y-1">
            ...
            {scenes.map(scene => (
              <button
                ...
                className="w-full text-left px-3 py-2.5 rounded-md hover:bg-sidebar-accent transition-colors"
              >
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="text-[10px] font-bold text-sidebar-primary bg-sidebar-accent px-1.5 py-0.5 rounded shrink-0">
                    {scene.number}
                  </span>
                  {scene.intExt && (
